...
export function parseDateRange(text) {
  ...
  if ((match = lower.match(/last\s+(\d+)\s+days?/))) {
    ...
    return { start: dateToYMD(start), end: todayStr, label: `最近 ${n} 天` };
  }
  ...
  if ((match = lower.match(/last\s+(\d+)\s+weeks?/))) {
    ...
    return { start: dateToYMD(start), end: todayStr, label: `最近 ${n} 週` };
  }
  ...
  if (lower.includes('this week')) {
    ...
    return { start: dateToYMD(start), end: todayStr, label: '本週' };
  }
  ...
  if (lower.includes('last week')) {
    ...
    return { start: dateToYMD(start), end: dateToYMD(end), label: '上週' };
  }

  if (lower.includes('this month')) {
    const start = new Date(now.getFullYear(), now.getMonth(), 1);
    return { start: dateToYMD(start), end: todayStr, label: '本月' };
  }

  if (lower.includes('last month')) {
    const start = new Date(now.getFullYear(), now.getMonth() - 1, 1);
    const end = new Date(now.getFullYear(), now.getMonth(), 0);
    return { start: dateToYMD(start), end: dateToYMD(end), label: '上個月' };
  }

  if (lower.includes('next month')) {
    const start = new Date(now.getFullYear(), now.getMonth() + 1, 1);
    const end = new Date(now.getFullYear(), now.getMonth() + 2, 0);
    return { start: dateToYMD(start), end: dateToYMD(end), label: '下個月' };
  }

  // 像 "february 2006" 或 "feb 2006" 這樣的 月份-年份
  ...
  if ((match = lower.match(/\b(\d{4})\b/))) {
    ...
    return { start: dateToYMD(start), end: dateToYMD(end), label: `${y} 年` };
  }

  // 僅月份，像 "february" 或 "feb" (假設為今年)
  if ((match = lower.match(/\b(january|february|march|april|may|june|july|august|september|october|november|december|jan|feb|mar|apr|may|jun|jul|aug|sep|sept|oct|nov|dec)\b/))) {
    ...
    if (mIdx !== undefined) {
      ...
      const label = `${now.getFullYear()} 年 ${mIdx + 1} 月`;
      return { start: dateToYMD(start), end: dateToYMD(end), label };
    }
  }
  ...
  const start = new Date();
  start.setDate(start.getDate() - 6);
  return { start: dateToYMD(start), end: todayStr, label: '最近 7 天' };
}
