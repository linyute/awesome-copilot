...
// PR 儀表板的獨立 CLI — 不需要 Copilot SDK。
// 用法：node pr-dashboard-cli.mjs [query] [role]
//   query：自然語言日期範圍，例如 "最近 2 週" (預設："最近 7 天")
//   role： "Authored by me" (我撰寫的) | "Requested reviews" (要求檢閱) | "Assigned to me" (指派給我) | "All" (全部) 之一
//          (預設："Authored by me")
...
const query = args[0] || "最近 7 天";
const role  = args[1] || "Authored by me";

// ── 工具函式 ───────────────────────────────────────────────────────────────────
function formatHumanDate(d) {
  try {
    return new Date(d).toLocaleDateString("zh-TW", { month: "short", day: "numeric", year: "numeric" });
  } catch (e) {
    return d;
  }
}
...
async function ghApi(args) {
  try {
    ...
    if (err?.code === "ENOENT") throw new Error("找不到 `gh` CLI。請安裝 GitHub CLI 並進行驗證 (gh auth login)。");
    ...
    throw new Error(`gh api 失敗：${errorMessage}`);
  }
}
...
async function getPrDetails(item) {
  try {
    ...
    if (out.draft) out.status = "草稿";
    else if (pr.merged) out.status = "已合併";
    else if (pr.state === "closed") out.status = "已關閉";
    ...
    // 透過 GitHub Markdown API 將第一段算繪為 HTML
    try {
...
function buildMarkdown(prs, label) {
  const open   = prs.filter(p => p.status === "開啟").length;
  const merged = prs.filter(p => p.status === "已合併").length;
  const closed = prs.filter(p => p.status === "已關閉").length;
  const draft  = prs.filter(p => p.status === "草稿").length;

  const lines = [`## PR 儀表板 — ${label}\n`,
    `**總計 ${prs.length} 個** · ✅ ${open} 開啟 · 🔀 ${merged} 已合併 · ❌ ${closed} 已關閉 · 📝 ${draft}\n\n`];
...
async function renderHtml(md, label = "PR 儀表板", prs = []) {
  const extDir = path.dirname(fileURLToPath(import.meta.url));
  const templatePath = path.join(extDir, "../assets/dashboard.html");
  let template = "";
  try { template = fs.readFileSync(templatePath, "utf8"); }
  catch (e) {
    template = `<html><head><title>PR 儀表板 — ${escapeHtml(label)}</title></head><body><pre>${escapeHtml(JSON.stringify(md))}</pre></body></html>`;
  }
  ...
  function statusColor(s) {
    return { 草稿: "#848d97", 已合併: "#6f42c1", 已關閉: "#da3633", 開啟: "#d29922" }[s] || "#848d97";
  }
  ...
  const rows = prs.map(pr => {
    const previewHtml = (pr.bodyHtml && String(pr.bodyHtml).trim())
      ...
      : pr.summary
        ...
        : `<div style="margin-top:6px;color:var(--muted);font-size:.9em">無說明</div>`;
  ...
  replaced = replaced.replace(/<span class="visible-count" id="vc">[^<]*<\/span>/, `<span class="visible-count" id="vc">${prs.length} 個 PR</span>`);

  try { replaced = replaced.replace(/<title>[^<]*<\/title>/, `<title>PR 儀表板 — ${escapeHtml(label)}</title>`); } catch (e) {}
  try { replaced = replaced.replace(/<h1[^>]*>[^<]*<\/h1>/, `<h1>🔀 PR 儀表板 — ${escapeHtml(label)}</h1>`); } catch (e) {}

  try {
    const nowStr = new Date().toLocaleString();
    replaced = replaced.replace(/<div class="meta">[^<]*<\/div>/, `<div class="meta">產生於 ${escapeHtml(nowStr)} · ${prs.length} 個 PR</div>`);
...
(async () => {
  try {
    ...
    console.log(`[pr-dashboard] 正在獲取 PR：${labelWithRange} · 角色：${role}`);
    ...
    console.log(`[pr-dashboard] 搜尋：${qstr}`);
    const items = await searchIssues(qstr);
    console.log(`[pr-dashboard] 找到 ${items.length} 個 PR`);

    if (!items.length) {
      const extDir = path.dirname(fileURLToPath(import.meta.url));
      const noResultsPath = path.join(os.tmpdir(), "pr-dashboard-no-results.html");
      fs.writeFileSync(noResultsPath,
        `<html><head><title>PR 儀表板 — ${labelWithRange}</title></head><body><h1>找不到 PR</h1><p>沒有符合您 ${labelWithRange} 查詢條件的 PR。</p></body></html>`,
        "utf8"
      );
      openInBrowser(noResultsPath);
      console.log("[pr-dashboard] 無結果 — 已開啟預留位置頁面。");
      return;
    }

    const prs = await pMap(items, getPrDetails, 5);
    console.log(`[pr-dashboard] 已獲取 ${prs.length} 個 PR 的詳細資訊`);

    const md  = buildMarkdown(prs, labelWithRange);
    const out = await renderHtml(md, labelWithRange, prs);
    openInBrowser(out);
    console.log(`[pr-dashboard] 儀表板已開啟：${out}`);
  } catch (e) {
    console.error("[pr-dashboard] 錯誤：", e?.message || e);
    process.exit(1);
  }
})();
