...
"""
Azure 互動式架構圖產生器 v3
產生帶有 Azure 官方圖示 (Base64 內嵌) 的互動式 HTML 圖表。
"""
...
# Azure 服務圖示：SVG、顏色 + 官方圖示金鑰對映
# icon: 48x48 viewBox SVG 路徑 (備用)
# azure_icon_key: icons.py 中的金鑰 (官方 Azure 圖示)
SERVICE_ICONS = {
    "openai": {
    ...
    "storage": {
        ...
        "color": "#0078D4", "bg": "#E8F4FD", "category": "資料",
    ...
    "fabric": {
        ...
        "color": "#E8740C", "bg": "#FEF3E8", "category": "資料",
    ...
    "keyvault": {
        ...
        "color": "#E8A000", "bg": "#FEF7E0", "category": "安全性",
    ...
    "vnet": {
        ...
        "color": "#5C2D91", "bg": "#F3EEF9", "category": "網路",
    ...
    "acr": {
        ...
        "color": "#0078D4", "bg": "#E8F4FD", "category": "運算"
    },
    "aks": {
        ...
        "color": "#326CE5", "bg": "#EBF0FC", "category": "運算",
    ...
    "appinsights": {
        ...
        "color": "#773ADC", "bg": "#F0EAFA", "category": "監視",
    ...
    "user": {
        ...
        "color": "#0078D4", "bg": "#E8F4FD", "category": "外部"
    },
    "app": {
        ...
        "color": "#666666", "bg": "#F5F5F5", "category": "外部"
    },
    "default": {
    ...
    "cdn": {
        ...
        "color": "#0078D4", "bg": "#E8F4FD", "category": "網路",
    ...
    "event_hub": {
        ...
        "color": "#0078D4", "bg": "#E8F4FD", "category": "整合",
    ...
    "redis": {
        ...
        "color": "#D83B01", "bg": "#FEF0E8", "category": "資料",
    ...
    "firewall": {
        ...
        "color": "#E8A000", "bg": "#FFF8E1", "category": "網路",
    ...
    "databricks": {
        ...
        "color": "#FF3621", "bg": "#FFF0EE", "category": "資料",
    ...
    "aks": {
        ...
        "color": "#326CE5", "bg": "#EBF0FA", "category": "運算",
        "azure_icon_key": "kubernetes_services"
    },
    "function_app": {
        ...
        "color": "#F0AD4E", "bg": "#FFF8ED", "category": "運算",
    ...
    "log_analytics": {
        ...
        "color": "#5C2D91", "bg": "#F3EDF7", "category": "監視",
    ...
    "sentinel": {
        ...
        "color": "#0078D4", "bg": "#E8F4FD", "category": "安全性",
    ...
    "spring_apps": {
        ...
        "color": "#6DB33F", "bg": "#EFF8E8", "category": "運算",
    ...
    "backup": {
        ...
        "color": "#0078D4", "bg": "#E8F4FD", "category": "管理",
...
<div class="container">
  <div class="canvas-area">
    <div class="toolbar">
      <button class="tool-btn" onclick="fitToScreen()">縮放至視窗</button>
      <div class="tool-sep"></div>
      <button class="tool-btn" onclick="zoomIn()">+</button>
      <button class="tool-btn" onclick="zoomOut()">&minus;</button>
      <div class="tool-sep"></div>
      <button class="tool-btn" onclick="resetZoom()">重設</button>
      <div class="tool-sep"></div>
    ...
    <div class="status-bar">拖曳節點 &middot; 捲動縮放 &middot; 拖曳空白處以平移</div>
  </div>
...
// ── 節點大小 ──
const SVC_W = 150, SVC_H = 100;  // 服務節點 (上方圖示，下方名稱)
const PE_W = 100, PE_H = 70;     // pe 節點 (較小)
const GAP = 40;

// ── 佈局：類別群組方框樣式 ──
// 每個類別都有一個具備標籤的方框，服務在內部以網格排列。
// 群組在 2D 排列：主要服務群組在頂部，底部群組在下方。
// PE 節點在獨立的 PE 子網路群組中。

const positions = {{}};
const useRgLayout = HIERARCHY.length > 0 && NODES.some(n => n.resourceGroup);
const peNodes = useRgLayout ? [] : NODES.filter(n => n.type === 'pe');  // RG 模式：PE 包含在 mainNodes 中
...
// ── 佈局策略：基於 RG (若有 HIERARCHY) 或基於類別 (預設) ──

if (useRgLayout) {{
  // ── 基於 RG 的佈局：依 訂閱 > 資源群組 進行分組 ──
  let gx = 60, gy = 140;
  let subStartX = 60;
  const SUB_GAP = 80;
  const RG_GAP = 60;
  ...
  if (unassigned.length > 0) {{
    ...
    groupBoxes.push({{
      cat: '其他', x: 60, y: bottomY, w: dim.w, h: dim.h,
      color: '#666'
    }});
  }}
...
// 動態加入任何不在 catOrder 或 bottomCategories 中的類別
...
let gx = 60, gy = 140;  // 服務群組的起始位置
let rowMaxH = 0;
let rowStartX = 60;
const MAX_ROW_W = Math.max(1600, serviceGroups.length * 400);  // 較寬以保持安全性與 AI/Data 並列
...
// ── PE 節點放置 ──
if (useRgLayout) {{
  // RG 模式：PE 節點進入其各自的 RG 方框中
  // 若 PE 具有 訂閱/資源群組，則其位置已由 RG 佈局設定
  // 對於未分配 RG 的 PE，將它們放在單獨的群組中
  ...
  if (unplacedPEs.length > 0) {{
    ...
    groupBoxes.push({{
      cat: '私用端點 (Private Endpoints)', x: peStartX, y: peStartY, w: peBoxW, h: peBoxH,
      color: '#5C2D91', isPE: true
    }});
  }}
}} else {{
  // 類別模式：PE 節點在服務群組上方的獨立群組中
  const PE_Y = 40;
  if (peNodes.length > 0) {{
    ...
    groupBoxes.push({{
      cat: '私用端點 (Private Endpoints)', x: 60, y: PE_Y, w: peBoxW, h: peBoxH,
      color: '#5C2D91', isPE: true
    }});
...
let draggingGroup = null, groupDragNodes = [];  // 用於 RG/群組方框拖曳
...
function renderDiagram() {{
  ...
  root.innerHTML = '';
  _routeCounter = 0;  // 每次轉譯時重設交錯計數器

  // ── 繪製 VNet 邊界 (僅在基於類別的佈局，而非 RG 佈局中) ──
  if (!useRgLayout) {{
  ...
  if (privateGroups.length > 0 && showVNet) {{
      ...
      const vnetLabel = VNET_INFO ? `虛擬網路 (${{VNET_INFO}})` : '虛擬網路';
  ...
  }} // VNet 邊界 if(!useRgLayout) 結束

  // ── 繪製群組方框 (類別或 RG — 取決於佈局模式) ──
  groupBoxes.forEach(gb => {{
    if (gb.isPE) {{
      // PE 群組 — 一律以虛線樣式繪製
    ...
    // 群組標籤 — RG 使用 📁，PE 使用 "私用端點"，類別使用類別名稱
    ...
    label.textContent = gb.isRG ? `📁 ${{gb.cat}}` : (gb.isPE ? '私用端點 (Private Endpoints)' : gb.cat);
  ...
  // ── 邊緣 (優先轉譯 — 節點轉譯在上方，覆蓋交差點) ──
  // 邊緣標籤會被收集並在節點之後轉譯，使其保持可見。
  // 僅限直角路由：具有直角轉向的水平/垂直線段。
  const _edgeLabels = [];
  EDGES.forEach(edge => {{
  ...
  NODES.forEach(node => {{
    ...
    if (!isPe) {{
      const TYPE_LABELS = {{
        ...
        'function_app': 'Function App', 'synapse': 'Synapse', 'vnet': '虛擬網路',
        'nsg': 'NSG', 'bastion': 'Bastion', 'pe': '私用端點',
...
// ── VNet 側邊欄卡片 (若 VNet 邊界存在則動態加入) ──
if (VNET_INFO || NODES.some(n => n.private && n.type !== 'pe') || NODES.some(n => n.type === 'pe')) {{
  const list = document.getElementById('service-list');
  // 插入在頂部
  const catLabel = document.createElement('div');
  catLabel.className = 'cat-label'; catLabel.textContent = '網路';
  const card = document.createElement('div');
...
def generate_diagram(services, connections, title="Azure 架構", vnet_info="", hierarchy=None):
    """產生一個作為 HTML 字串的互動式 Azure 架構圖。

    引數：
        services: 包含 id, name, type, sku, private, details 等金鑰的字典清單。
        connections: 包含 from, to, label, type 金鑰的字典清單。
        title: 圖表標題字串。
        vnet_info: VNet CIDR 資訊字串。
        hierarchy: 選用的訂閱/資源群組 (RG) 階層清單。
