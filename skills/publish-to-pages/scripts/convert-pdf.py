#!/usr/bin/env python3
"""將 PDF 轉換為自帶內容的 HTML 簡報。

每一頁都會轉譯為 PNG 圖片 (透過 pdftoppm) 並以 Base64 內嵌
到單一 HTML 檔案中，且具有投影片導覽功能 (箭頭、滑動、點擊)。

需求：poppler-utils (pdftoppm)
用法：python3 convert-pdf.py input.pdf [output.html]
"""
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for i, page_path in enumerate(pages, 1):
            ...
            slides_html.append(
                ...
                f'<img src="data:image/png;base64,{b64}" alt="第 {i} 頁">'
                f'</div></section>'
            )
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("用法：convert-pdf.py <file.pdf> [output.html]")
        sys.exit(1)
