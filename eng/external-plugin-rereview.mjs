...
export function removePluginFromExternalJson({ pluginName, sourceRepo, filePath = EXTERNAL_PLUGINS_FILE } = {}) {
  ...
  if (matchIndex === -1) {
    throw new Error(`在 ${path.relative(process.cwd(), filePath)} 中找不到外部外掛程式 "${pluginName || sourceRepo}"`);
  }
...
if (isCli) {
  ...
  if (command !== "remove") {
    console.error("用法: node ./eng/external-plugin-rereview.mjs remove --plugin-name <名稱> [--source-repo <擁有者/存放庫>] [--file <路徑>]");
    process.exit(1);
  }
  ...
  if (!args["plugin-name"] && !args["source-repo"]) {
    console.error("移除外部外掛程式時，請提供 --plugin-name 或 --source-repo。");
    process.exit(1);
  }
