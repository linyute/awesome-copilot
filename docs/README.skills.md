...
| [appinsights-instrumentation](../skills/appinsights-instrumentation/SKILL.md) | 為應用程式安裝檢測以將有用的遙測資料傳送至 Azure App Insights | `LICENSE.txt`<br />`examples`<br />`references/ASPNETCORE.md`<br />`references/AUTO.md`<br />`references/NODEJS.md`<br />`references/PYTHON.md`<br />`scripts/appinsights.ps1` |
...
| [automate-this](../skills/automate-this/SKILL.md) | 分析手動程序的操作側錄，並產出針對性的、可運行的自動化指令碼。從影片檔案中擷取畫面與音訊旁白，重建逐步工作流程，並使用使用者機器上已安裝的工具建議多個複雜層級的自動化。 | 無 |
...
| [creating-oracle-to-postgres-master-migration-plan](../skills/creating-oracle-to-postgres-master-migration-plan/SKILL.md) | 探索 .NET 解決方案中的所有專案，將每個專案分類為 Oracle 到 PostgreSQL 遷移資格，並產出持久性的主遷移計畫。在啟動多專案 Oracle 到 PostgreSQL 遷移、建立遷移清單或評估哪些 .NET 專案包含 Oracle 相依性時使用。 | 無 |
| [creating-oracle-to-postgres-migration-bug-report](../skills/creating-oracle-to-postgres-migration-bug-report/SKILL.md) | 針對 Oracle 到 PostgreSQL 遷移期間發現的缺陷建立結構化錯誤報告。在將 Oracle 和 PostgreSQL 之間的行為差異記錄為具有嚴重性、根本原因和修復步驟的可採取行動錯誤報告時使用。 | `references/BUG-REPORT-TEMPLATE.md` |
| [creating-oracle-to-postgres-migration-integration-tests](../skills/creating-oracle-to-postgres-migration-integration-tests/SKILL.md) | 在 Oracle 到 PostgreSQL 資料庫遷移期間，為 .NET 資料存取成品建立整合測試案例。產生具有決定性種子資料且與資料庫無關的 xUnit 測試，以驗證兩個資料庫系統之間的行為一致性。在為遷移專案建立整合測試、為資料存取層產生測試涵蓋範圍或撰寫 Oracle 到 PostgreSQL 遷移驗證測試時使用。 | 無 |
...
| [doublecheck](../skills/doublecheck/SKILL.md) | 三層驗證管線，適用於 AI 輸出。擷取可驗證的主張，透過網路搜尋尋找支持或矛盾的來源，針對幻覺模式執行對抗性審核，並產生包含供人工審核之來源連結的結構化驗證報告。 | `assets/verification-report-template.md` |
...
| [github-copilot-starter](../skills/github-copilot-starter/SKILL.md) | 根據技術堆疊為新專案設定完整的 GitHub Copilot 設定 | 無 |
| [github-issues](../skills/github-issues/SKILL.md) | 使用 MCP 工具建立、更新與管理 GitHub 議題。當使用者想要建立錯誤回報、功能請求或任務議題、更新現有議題、加入標籤/指派者/里程碑、設定議題欄位（日期、優先權、自訂欄位）、指定議題類型、管理議題工作流程、連結議題、加入相依性或追蹤「被阻擋/阻擋」關係時使用。觸發範例："建立議題"、"回報錯誤"、"請求功能"、"更新議題 X"、"設定優先權"、"設定開始日期"、"連結議題"、"加入相依性"、"被阻擋"、"阻擋"。 | `references/dependencies.md`<br />`references/images.md`<br />`references/issue-fields.md`<br />`references/issue-types.md`<br />`references/projects.md`<br />`references/search.md`<br />`references/sub-issues.md`<br />`references/templates.md` |
...
| [napkin](../skills/napkin/SKILL.md) | 針對 Copilot CLI 的視覺化白板協作。建立一個在瀏覽器中開啟的互動式白板 — 繪圖、素描、加入便籤，然後將所有內容分享回 Copilot。Copilot 會看到您的圖圖畫畫與文字，並以分析、建議和構思作為回應。 | `assets/napkin.html`<br />`assets/step1-activate.svg`<br />`assets/step2-whiteboard.svg`<br />`assets/step3-draw.svg`<br />`assets/step4-share.svg`<br />`assets/step5-response.svg` |
...
| [planning-oracle-to-postgres-migration-integration-testing](../skills/planning-oracle-to-postgres-migration-integration-testing/SKILL.md) | 為 Oracle 到 PostgreSQL 資料庫遷移期間的 .NET 資料存取成品建立整合測試計畫。分析單一專案以識別與資料庫互動的存放庫、DAO 和服務層，然後產出結構化測試計畫。在規劃遷移專案的整合測試涵蓋範圍、識別哪些資料存取方法需要測試或為 Oracle 到 PostgreSQL 遷移驗證做準備時使用。 | 無 |
...
| [webapp-testing](../skills/webapp-testing/SKILL.md) | 使用 Playwright 與本地 Web 應用程式互動並進行測試的工具組。支援驗證前端功能、除錯 UI 行為、擷取瀏覽器螢幕截圖以及查看瀏覽器紀錄。 | `assets/test-helper.js`<br />`test-helper.js` |
...
| [winmd-api-search](../skills/winmd-api-search/SKILL.md) | 尋找並探索 Windows 桌面 API。在建構需要平台能力的特性時使用 —— 如相機、檔案存取、通知、UI 控制項、AI/ML、感測器、網路等。探索適用於特定任務的 API 並獲取完整的類型細節 (方法、屬性、事件、列舉值)。 | `LICENSE.txt`<br />`scripts/Invoke-WinMdQuery.ps1`<br />`scripts/Update-WinMdCache.ps1`<br />`scripts/cache-generator` |
