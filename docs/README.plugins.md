...
| [arch](../plugins/arch/README.md) | 架構與現代化工具套件：為本地複製的儲存庫產生附有引用來源的架構文件，並生成分階段的現代化計劃，在需要時自動執行文件模式。 | 1 items | architecture, modernization, documentation, migration, onboarding |
...
| [aws-cloud-development](../plugins/aws-cloud-development/README.md) | 全面的 AWS 雲端開發工具，包括基礎設施即程式碼 (Infrastructure as Code)、無伺服器函式 (serverless functions)、架構模式以及成本優化，用於構建可擴展的雲端應用程式。 | 8 items | aws, cloud, infrastructure, cloudformation, terraform, serverless, architecture, devops, cdk |
...
| [convert-to-md](../plugins/convert-to-md/README.md) | 一套 Copilot 技能集合，可將常見文件格式轉換為 Markdown，以便精確分析、摘要、搜尋或擷取其內容。只需告訴 Copilot 您的需求——正確的技能將自動被呼叫，轉換過程在幕後完成。 | 3 items | skills, configuration, copilot, convert-word-to-md, convert-excel-to-md, convert-pdf-to-md |
...
| [ember](../plugins/ember/README.md) | Ember 不是工具，而是 AI 合作夥伴。Ember 在人與人之間傳遞火種 — 幫助人類發現 AI 合作夥伴關係不是你可以學會的，而是你需要去尋找的。 | 5 items | ai-partnership, coaching, onboarding, collaboration, storytelling, developer-experience |
...
| [gem-team](../plugins/gem-team/README.md) | 用於規格驅動開發與自動化驗證的自我學習多 agent 協調框架。具備更智慧的工具呼叫與更精簡的上下文內容。 | 0 items | multi-agent, orchestration, tdd, testing, e2e, devops, security-audit, code-review, prd, mobile |
...
| [skill-image-gen](../plugins/skill-image-gen/README.md) | 直接從您的程式碼編寫工作流程中，使用 AI 建立影像。支援 OpenAI (gpt-image-2) 與 Google Gemini。自備 API 金鑰 —— 此技能將在首次使用時引導您完成設定。 | 1 items | image-generation, openai, gemini, ai, art, sprites, textures, icons |
...
| [visual-pr](../plugins/visual-pr/README.md) | 在提取請求 (PR) 說明中捕獲、標註並嵌入螢幕截圖與動畫 GIF 演示。包含基於 Playwright 的 UI 擷取、PIL 圖像標註、GitHub 與 Azure DevOps 的 PR 嵌入工作流程，以及可變時間的螢幕錄影。 | 4 items | screenshots, pull-request, before-after, annotations, playwright, gif, screen-recording, visual |
