...
2. **掃描本地自訂聊天模式**：在 `.github/chatmodes/` 資料夾中發現現有的自訂聊天模式檔案
...
10. **下載資產**：對於請求的聊天模式，自動下載並將個別聊天模式安裝到 `.github/chatmodes/` 資料夾。請勿調整檔案內容。使用 `#todos` 工具追蹤進度。優先使用 `#fetch` 工具下載資產，但可以使用 `#runInTerminal` 工具中的 `curl` 來確保檢索所有內容。
...
|---------------------------|-------------|-------------------|-------------------------|---------------------|
| [code-reviewer.chatmode.md](https://github.com/github/awesome-copilot/blob/main/chatmodes/code-reviewer.chatmode.md) | 專門的程式碼審查自訂聊天模式 | ❌ 否 | 無 | 將透過專用的程式碼審查協助增強開發工作流程 |
...
| [debugging-expert.chatmode.md](https://github.com/github/awesome-copilot/blob/main/chatmodes/debugging-expert.chatmode.md) | 偵錯協助自訂聊天模式 | ❌ 否 | 無 | 可以提高開發團隊的故障排除效率 |

## 本地聊天模式發現處理程序

1. 列出 `.github/chatmodes/` 目錄中的所有 `*.chatmode.md` 檔案
...
- 掃描本地檔案系統以查找 `.github/chatmodes/` 目錄中現有的聊天模式
