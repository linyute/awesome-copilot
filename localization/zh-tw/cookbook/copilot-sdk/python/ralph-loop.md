...
> **可執行的範例：** [recipe/ralph_loop.py](recipe/ralph_loop.py)
>
> 從儲存庫根目錄安裝相依套件並執行：
>
> ```bash
...
> ```
>
> 在執行迴圈之前，請確保目前工作目錄中存在 `PROMPT_build.md` 和 `PROMPT_plan.md`。
...
9. **設定 `working_directory`**：將工作階段固定在您的專案根目錄，以便工具操作能正確解析路徑
10. **自動核准權限**：使用 `on_permission_request` 允許工具呼叫，而不中斷迴圈
