...
一個由社群建立的自訂代理程式 (agents)、指示 (instructions)、技能 (skills)、掛鉤 (hooks)、工作流程 (workflows) 和外掛程式 (plugins) 集合，旨在增強您的 GitHub Copilot 體驗。
...
剛開始接觸 GitHub Copilot 自訂功能？網站上的 **[學習中心 (Learning Hub)](https://awesome-copilot.github.com/learning-hub)** 提供精選文章、逐步解說和參考資料 — 涵蓋從代理程式、技能和指示等核心概念，到掛鉤、代理工作流程、MCP 伺服器和 Copilot 編碼代理程式的實作指南。

## 此存放庫中的內容

| 資源                                                           | 說明                                             | 瀏覽                                                           |
| -------------------------------------------------------------- | ------------------------------------------------ | -------------------------------------------------------------- |
| 🤖 [代理程式 (Agents)](docs/README.agents.md)                   | 與 MCP 伺服器整合的專用 Copilot 代理程式         | [所有代理程式 →](https://awesome-copilot.github.com/agents)    |
...
對於大多數使用者，**Awesome Copilot** 市集已在 Copilot CLI/VS Code 中註冊，因此您可以直接安裝外掛程式：

```bash
copilot plugin install <plugin-name>@awesome-copilot
```

如果您使用的是舊版 Copilot CLI 或自訂設定，並且看到市集未知的錯誤，請先註冊一次然後安裝：
...
參閱 [CONTRIBUTING.md](CONTRIBUTING.md) · [AGENTS.md](AGENTS.md) 了解 AI 代理程式指南 · [安全性 (Security)](SECURITY.md) · [行為準則 (Code of Conduct)](CODE_OF_CONDUCT.md)

> 這裡的自訂功能來自第三方開發人員。安裝前請檢查任何代理程式及其文件。

## 貢獻者 ✨

感謝這些優秀的人士 ([表情符號說明](./CONTRIBUTING.md#contributors-recognition)):
...
- [VS Code Copilot 自訂文件](https://code.visualstudio.com/docs/copilot/copilot-customization) - Microsoft 官方文件
...
此專案可能包含專案、產品或服務的商標或標誌。Microsoft 商標或標誌的
授權使用受 [Microsoft 商標與品牌指南](https://www.microsoft.com/en-us/legal/intellectualproperty/trademarks/usage/general)
規範，且必須遵守該指南。
在修改版的此專案中使用 Microsoft 商標或標誌不得引起混淆或暗示 Microsoft 的贊助。
