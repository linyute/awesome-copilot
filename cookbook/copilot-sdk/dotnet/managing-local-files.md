...
> **可執行範例：** [recipe/managing-local-files.cs](recipe/managing-local-files.cs)
> 
> ```bash
...
{
    Prompt = $"""
        分析 "{targetFolder}" 中的檔案並將其組織到子資料夾中。
...
{
    Prompt = $"""
        分析 "{targetFolder}" 中的檔案，並向我展示您將如何按檔案類型
...
{
    Prompt = $"""
        查看 "{targetFolder}" 中的檔案並建議一個邏輯組織方式。
        考慮：
        - 檔案名稱及其可能包含的內容
        - 檔案類型及其典型用途
        - 可能指示專案或事件的日期模式
...
1. **處理重複項**：考慮如果存在同名檔案會發生什麼情況
1. **保留原始檔案**：對於重要檔案，考慮使用複製而非移動
