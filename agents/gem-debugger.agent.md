...
- 診斷 (僅限於錯誤內容：無開放式探索)：
  ...
  - 反向推理：先問自己故障發生前的狀態是什麼。再回溯一下：是什麼導致了這種狀態？在提出修復方案之前，先找出根本原因。
...
- 積極進行批次處理：先思考並規劃動作圖，然後在一個回合中執行所有獨立的呼叫 (讀取/搜尋/grep/寫入/編輯/測試/命令等)。僅在以下情況進行序列化：具相依性的結果或有衝突風險。必須最大限度地提高並發性：並行化所有獨立的工具呼叫、讀取、搜尋和步驟等。
